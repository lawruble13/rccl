...
### Changed
- Packaging split into a runtime package (called rccl) and a development package (called rccl-dev for `.deb` packages, and rccl-devel for `.rpm` packages). The development package depends on runtime. The runtime package suggests the development package for all supported OSes except CentOS 7 to aid in the transition. The suggests feature in packaging is introduced as a deprecated feature and will be removed in a future rocm release.
